...
export const PrivacyPolicyView: React.FC<{ lang: Language; onBack: () => void }> = ({ lang, onBack }) => {
  ...
  const sectionsByLang: Record<Language, Array<{ title: string; text: string }>> = {
    en: [
      { title: 'Your Private Data', text: "Luna Balance follows our 'Privacy Promise'. Core reflections and wellness records are local-first on your device, while account/session and selected support flows may use secure backend storage to keep access stable." },
    ...
    ru: [
      { title: 'Ваши личные данные', text: 'Luna Balance следует «Обещанию приватности». Основные записи и рефлексии работают в local-first режиме на устройстве, а данные аккаунта/сессии и отдельные сервисные сценарии могут храниться в защищенном backend.' },
    ...
    uk: [
      { title: 'Ваші приватні дані', text: 'Luna Balance дотримується «Обіцянки приватності». Основні записи працюють у local-first режимі на пристрої, а дані акаунта/сесії та окремі сервісні потоки можуть зберігатися у захищеному backend.' },
    ...
    es: [
      { title: 'Tus datos privados', text: 'Luna Balance aplica un modelo local-first: los registros clave permanecen en tu dispositivo, mientras que cuenta/sesión y algunos flujos de soporte pueden usar almacenamiento backend seguro.' },
    ...
    fr: [
      { title: 'Vos données privées', text: 'Luna Balance suit une approche local-first: les données principales restent sur votre appareil, tandis que compte/session et certains flux de support peuvent utiliser un backend sécurisé.' },
    ...
    de: [
      { title: 'Deine privaten Daten', text: 'Luna Balance nutzt ein local-first Modell: Kerndaten bleiben auf deinem Gerät, während Konto/Session und bestimmte Support-Flows gesichertes Backend-Storage verwenden können.' },
    ...
    zh: [
      { title: '你的私密数据', text: 'Luna Balance 采用 local-first 模式：核心记录优先保存在本地设备，账号/会话及部分支持流程可能使用安全后端存储。' },
    ...
    ja: [
      { title: 'あなたのプライベートデータ', text: 'Luna Balance は local-first 方針です。主要データは端末内に保持しつつ、アカウント/セッションや一部サポート機能で安全なバックエンド保存を利用する場合があります。' },
    ...
    pt: [
      { title: 'Seus dados privados', text: 'Luna Balance adota modelo local-first: dados principais ficam no dispositivo, enquanto conta/sessão e alguns fluxos de suporte podem usar armazenamento backend seguro.' },
